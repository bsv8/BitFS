...
export function registerShellIPC(
  ...
  ipcMain.handle("bitfs-viewer:wallet-token-send-preview", (_event, payload: { tokenStandard?: string; token_standard?: string; assetKey?: string; asset_key?: string; amountText?: string; amount_text?: string; toAddress?: string; to_address?: string }) => {
    debugLogger.log("ipc", "viewer_wallet_token_send_preview", {
      ...
      asset_key: String(payload?.assetKey || payload?.asset_key || ""),
      amount_text: String(payload?.amountText || payload?.amount_text || ""),
      to_address: String(payload?.toAddress || payload?.to_address || "")
    });
    return runtime.previewPublicWalletTokenSend(payload || {});
  });
  ipcMain.handle("bitfs-viewer:wallet-token-send-sign", (_event, payload: { tokenStandard?: string; token_standard?: string; assetKey?: string; asset_key?: string; amountText?: string; amount_text?: string; toAddress?: string; to_address?: string; expectedPreviewHash?: string; expected_preview_hash?: string }) => {
    debugLogger.log("ipc", "viewer_wallet_token_send_sign", {
      token_standard: String(payload?.tokenStandard || payload?.token_standard || ""),
      asset_key: String(payload?.assetKey || payload?.asset_key || ""),
      amount_text: String(payload?.amountText || payload?.amount_text || ""),
      to_address: String(payload?.toAddress || payload?.to_address || ""),
    ...
    return runtime.signPublicWalletTokenSend(payload || {});
  });
  ipcMain.handle("bitfs-viewer:wallet-token-send-submit", (_event, payload: { signedTxHex?: string; signed_tx_hex?: string }) => {
    debugLogger.log("ipc", "viewer_wallet_token_send_submit", {
