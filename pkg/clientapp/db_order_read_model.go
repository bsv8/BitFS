...
// 业务结算聚合读模型（第四步主线）
// 设计原则：
//   - 业务状态优先看 order_settlements，不看旧过程表
//   - 汇总状态只根据 order_settlements 计算，不参考 proc_direct_transfer_pools/txid/旧表
...
// SettlementSummary 汇总状态（只根据 order_settlements 计算）
...
func ListBusinessesWithSettlementsByFrontOrderID(ctx context.Context, store *clientDB, frontOrderID string) ([]BusinessSettlementView, error) {
	...
	for _, biz := range businesses {
		...
		// 2. 查 settlement
		settlement, err := dbGetBusinessSettlementByBusinessID(ctx, store, biz.OrderID)
		if err != nil {
		...
		if view.Settlement.SettlementMethod == string(SettlementMethodChain) && view.Settlement.Status == "settled" && view.Settlement.TargetID != "" {
			...
			pa, err := GetPoolAllocationByBusinessID(ctx, store, biz.OrderID)
			if err == nil {
...
// computeSettlementSummary 只根据 order_settlements 计算汇总状态
// 规则：
//   - 全部 settled → settled
//   - 全部 failed → failed
//   - 全部 pending → pending
