...
func createLegacyCommandFactTablesWithTransitionColumns(t *testing.T, db *sql.DB) {
	...
	stmts := []string{
		`CREATE TABLE proc_command_journal(
		...
		`CREATE TABLE proc_domain_events(
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			created_at_unix INTEGER NOT NULL,
		...
		`CREATE TABLE proc_state_snapshots(
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			created_at_unix INTEGER NOT NULL,
			command_id TEXT NOT NULL,
			gateway_pubkey_hex TEXT NOT NULL,
...
func TestCommandFactFinalMigrationRemovesTransitionColumnsAndDropsObservedRows(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO proc_domain_events(created_at_unix,command_id,gateway_pubkey_hex,source_kind,source_ref,observed_at_unix,event_name,state_before,state_after,payload_json)
		VALUES(?,?,?,?,?,?,?,?,?,?)`,
	...
	if _, err := db.Exec(`INSERT INTO proc_state_snapshots(created_at_unix,command_id,gateway_pubkey_hex,source_kind,source_ref,observed_at_unix,state,pause_reason,pause_need_satoshi,pause_have_satoshi,last_error,payload_json)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
	...
	var count int
	if err := db.QueryRow(`SELECT COUNT(1) FROM proc_domain_events`).Scan(&count); err != nil {
		t.Fatalf("count proc_domain_events failed: %v", err)
	}
	if count != 1 {
		t.Fatalf("unexpected proc_domain_events count after migration: got=%d want=1", count)
	}
	if err := db.QueryRow(`SELECT COUNT(1) FROM proc_state_snapshots`).Scan(&count); err != nil {
		t.Fatalf("count proc_state_snapshots failed: %v", err)
	}
	if count != 1 {
		t.Fatalf("unexpected proc_state_snapshots count after migration: got=%d want=1", count)
	}

	var commandID string
	if err := db.QueryRow(`SELECT command_id FROM proc_domain_events ORDER BY id ASC LIMIT 1`).Scan(&commandID); err != nil {
	...
	if err := db.QueryRow(`SELECT command_id FROM proc_state_snapshots ORDER BY id ASC LIMIT 1`).Scan(&commandID); err != nil {
...
func TestObservedGatewayStatePayloadMigrationUsesFinalEnvelope(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO proc_observed_gateway_states(
		...
		1700000301, "gw1", "gw1", 1700000301, "fee_pool_resumed_by_wallet_probe", "paused_insufficient", "idle", "", 0, 999999, "", `{"source_table":"proc_state_snapshots","snapshot_payload":{"legacy":true}}`,
	); err != nil {
	...
	if err := db.QueryRow(`SELECT payload_json FROM proc_observed_gateway_states WHERE gateway_pubkey_hex=?`, "gw1").Scan(&payloadJSON); err != nil {
	...
	if payload.Extra == nil || payload.Extra["source_table"] != "proc_state_snapshots" {
