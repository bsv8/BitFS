...
func TriggerDomainRegisterLock(ctx context.Context, store BusinessStore, rt RuntimePorts, p TriggerDomainRegisterLockParams) (TriggerDomainRegisterLockResult, error) {
	...
	resolverPubkeyHex, resolverPeerID, err := rt.EnsureDomainPeerConnected(ctx, p.ResolverPubkeyHex, p.ResolverAddr)
	if err != nil {
		return out, err
	}
	...
	queryOut, err := rt.TriggerDomainQueryName(ctx, resolverPubkeyHex, resolverPeerID, name)
	if err != nil {
	...
	lockResp, err := rt.TriggerDomainRegisterLock(ctx, resolverPubkeyHex, name, targetPubkeyHex, queryResp.RegisterLockFee)
	if err != nil {
	...
	quote, err := rt.VerifyRegisterQuote(resolverPubkeyHex, lockResp.SignedQuoteJSON)
	if err != nil {
		return out, err
	}
...
func TriggerDomainPrepareRegister(ctx context.Context, store BusinessStore, rt RuntimePorts, p TriggerDomainPrepareRegisterParams) (TriggerDomainPrepareRegisterResult, error) {
	...
	if rt.WalletAllocMutex() != nil {
		rt.WalletAllocMutex().Lock()
		defer rt.WalletAllocMutex().Unlock()
	}

	built, err := rt.BuildDomainRegisterTx(ctx, lockResp.SignedQuoteJSON, quote)
	if err != nil {
		return out, err
	}
	out.Ok = true
...
// TriggerDomainSubmitPreparedRegister 提交已准备好的正式注册交易。
func TriggerDomainSubmitPreparedRegister(ctx context.Context, store BusinessStore, rt RuntimePorts, p TriggerDomainSubmitPreparedRegisterParams) (TriggerDomainSubmitPreparedRegisterResult, error) {
	...
	submitResp, err := rt.TriggerDomainRegisterSubmit(ctx, strings.TrimSpace(p.ResolverPubkeyHex), registerTxRaw)
	if err != nil {
	...
	if err := rt.ApplyLocalBroadcastWalletTxBytes(ctx, registerTxRaw, "domain_register_submit"); err != nil {
		return out, err
	}
	if err := rt.RecordChainPaymentAccountingAfterBroadcast(ctx, registerTxRaw, out.RegisterTxID, "domain_register", "domain_register_submit", "client:self", "resolver:"+strings.ToLower(strings.TrimSpace(p.ResolverPubkeyHex))); err != nil {
		return out, err
	}
	out.Ok = true
	out.Code = "OK"
...
func TriggerDomainSetTarget(ctx context.Context, store BusinessStore, rt RuntimePorts, p TriggerDomainSetTargetParams) (TriggerDomainSetTargetResult, error) {
	...
	resp, err := rt.TriggerDomainSetTarget(ctx, resolverPubkeyHex, name, targetPubkeyHex, queryResp.SetTargetFee)
	if err != nil {
		return out, err
	}
