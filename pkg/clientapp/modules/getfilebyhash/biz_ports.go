...
type QuoteReader interface {
	...
	ListQuotes(ctx context.Context, demandID string) ([]DirectQuote, error)
}

// DirectQuote 来自 seller 的直接报价
type DirectQuote struct {
...
type DownloadPolicy interface {
	// ShouldSelectChunk 判断某 chunk 是否应选中该 seller
	ShouldSelectChunk(ctx context.Context, chunkIndex uint32, sellerPubkey string, chunkPriceSat uint64) (bool, string)
	// ShouldContinueDownload 判断是否继续下载
	ShouldContinueDownload(ctx context.Context, jobID string, totalPaidSat uint64) (bool, string)
}
