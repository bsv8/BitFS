...
func applyLocalBroadcastWalletTx(ctx context.Context, store *clientDB, rt *Runtime, txHex string, trigger string) error {
	if store == nil {
...
func applyLocalBroadcastWalletTxBytes(ctx context.Context, store *clientDB, rt *Runtime, rawTx []byte, trigger string) error {
	if len(rawTx) == 0 {
...
func applyLocalBroadcastWalletProjection(ctx context.Context, store *clientDB, rt *Runtime, tx *txsdk.Transaction, trigger string) error {
