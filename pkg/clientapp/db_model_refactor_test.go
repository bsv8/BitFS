...
func TestClientDBLegacyMigrationAndSystemPricingFill(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO biz_workspaces(workspace_path,path,max_bytes,enabled,created_at_unix,updated_at_unix) VALUES(?,?,?,?,?,?)`, wsRoot, wsRoot, int64(4096), int64(1), int64(11), int64(12)); err != nil {
		t.Fatalf("insert legacy workspace: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO biz_workspace_files(workspace_path,file_path,path,seed_hash,seed_locked,file_size,mtime_unix,updated_at_unix) VALUES(?,?,?,?,?,?,?,?)`, wsRoot, "file-a.bin", filepath.Join(wsRoot, "file-a.bin"), seed1, int64(0), int64(123), int64(22), int64(23)); err != nil {
		t.Fatalf("insert legacy workspace file a: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO biz_workspace_files(workspace_path,file_path,path,seed_hash,seed_locked,file_size,mtime_unix,updated_at_unix) VALUES(?,?,?,?,?,?,?,?)`, wsRoot, "file-b.bin", filepath.Join(wsRoot, "file-b.bin"), seed2, int64(1), int64(456), int64(33), int64(34)); err != nil {
		t.Fatalf("insert legacy workspace file b: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO biz_seeds(seed_hash,chunk_count,file_size,seed_file_path,recommended_file_name,mime_hint,created_at_unix) VALUES(?,?,?,?,?,?,?)`, seed1, int64(3), int64(300), filepath.Join(base, "biz_seeds", "seed1.bse"), "file-a.bin", "text/plain", int64(44)); err != nil {
		t.Fatalf("insert legacy seed 1: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO biz_seeds(seed_hash,chunk_count,file_size,seed_file_path,recommended_file_name,mime_hint,created_at_unix) VALUES(?,?,?,?,?,?,?)`, seed2, int64(2), int64(200), filepath.Join(base, "biz_seeds", "seed2.bse"), "file-b.bin", "text/plain", int64(55)); err != nil {
	...
	cols, err := tableColumns(db, "biz_seed_chunk_supply")
	if err != nil {
		t.Fatalf("read chunk supply columns: %v", err)
	}
	if len(cols) != 2 {
		t.Fatalf("biz_seed_chunk_supply column count mismatch: got=%d want=2", len(cols))
	}
	if _, ok := cols["seed_hash"]; !ok {
		t.Fatalf("biz_seed_chunk_supply missing seed_hash")
	}
	if _, ok := cols["chunk_index"]; !ok {
		t.Fatalf("biz_seed_chunk_supply missing chunk_index")
	}
	if _, ok := cols["chunk_size"]; ok {
		t.Fatalf("biz_seed_chunk_supply should not keep chunk_size")
	}
	if _, ok := cols["chunk_hash"]; ok {
		t.Fatalf("biz_seed_chunk_supply should not keep chunk_hash")
	}

	var filePath string
	var locked int64
	if err := db.QueryRow(`SELECT file_path,seed_locked FROM biz_workspace_files WHERE workspace_path=? AND seed_hash=?`, wsRoot, seed1).Scan(&filePath, &locked); err != nil {
	...
	var count int
	if err := db.QueryRow(`SELECT COUNT(1) FROM biz_seed_chunk_supply WHERE seed_hash=?`, seed1).Scan(&count); err != nil {
		t.Fatalf("count migrated supply: %v", err)
	}
	if count != 2 {
		t.Fatalf("migrated supply count mismatch: got=%d want=2", count)
	}
	if err := db.QueryRow(`SELECT COUNT(1) FROM biz_seed_chunk_supply WHERE seed_hash=?`, seed2).Scan(&count); err != nil {
	...
	var source string
	if err := db.QueryRow(`SELECT floor_unit_price_sat_per_64k,resale_discount_bps,pricing_source FROM biz_seed_pricing_policy WHERE seed_hash=?`, seed1).Scan(&floor, &resale, &source); err != nil {
	...
	if err := db.QueryRow(`SELECT COUNT(1) FROM biz_seed_pricing_policy WHERE seed_hash=?`, seed2).Scan(&count); err != nil {
	...
	if err := db.QueryRow(`SELECT floor_unit_price_sat_per_64k,resale_discount_bps,pricing_source FROM biz_seed_pricing_policy WHERE seed_hash=?`, seed2).Scan(&floor, &resale, &source); err != nil {
