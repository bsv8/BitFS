...
func TestReconcileWalletUTXOSet_PreservesPendingLocalBroadcastWhenUpstreamLags(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT COUNT(1) FROM fact_settlement_payment_attempts WHERE source_type='chain_direct_pay' AND source_id=?`, localTxID).Scan(&paymentAttemptCount); err != nil {
		t.Fatalf("query fact_settlement_payment_attempts count: %v", err)
	}
	if paymentAttemptCount != 0 {
		t.Fatalf("unexpected fact_settlement_payment_attempts rows for local projection, got=%d", paymentAttemptCount)
	}
	var factCount int
