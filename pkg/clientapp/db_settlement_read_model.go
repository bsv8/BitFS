...
// 业务结算聚合读模型（第四步主线）
// 设计原则：
//   - 业务状态优先看 business_settlements，不看旧过程表
//   - 汇总状态只根据 business_settlements 计算，不参考 direct_transfer_pools/txid/旧表
...
// SettlementSummary 汇总状态（只根据 business_settlements 计算）
...
func GetFrontOrderSettlementSummary(ctx context.Context, store *clientDB, frontOrderID string) (FrontOrderSettlementSummary, error) {
	...
	// 3. 计算汇总状态（只看 business_settlements）
...
// computeSettlementSummary 只根据 business_settlements 计算汇总状态
// 规则：
//   - 全部 settled → settled
//   - 全部 failed → failed
//   - 全部 pending → pending
	...
	return clientDBValue(ctx, store, func(db *sql.DB) (BusinessSettlementItem, error) {
		...
		err := db.QueryRow(
			`SELECT settlement_id,business_id,settlement_method,status,target_type,target_id,error_message,created_at_unix,updated_at_unix,payload_json
			 FROM business_settlements WHERE settlement_method='chain' AND target_id=?`,
