...
// ============================================================
// 第五步：历史回填工具
// 职责：把旧表历史数据补录到新主线（front_orders -> business -> settlement）
// 原则：
//   - 幂等：重复执行不产生重复数据
//   - 不覆盖：已存在的新主线数据不被覆盖
...
// 策略：
//   - 查旧表 chain_payments 中 subtype='domain_register' 的记录
...
func BackfillDomainRegisterHistory(ctx context.Context, store *clientDB) (*BackfillResult, error) {
	...
	err := store.Do(ctx, func(db *sql.DB) error {
		// 查询所有域名注册相关的 chain_payments，且还没有对应 settlement 的
		rows, err := db.Query(`
			...
			FROM chain_payments cp
			LEFT JOIN business_settlements bs ON bs.settlement_method='chain' 
		...
		`)
		if err != nil {
			return fmt.Errorf("query chain_payments: %w", err)
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			_ = db.QueryRow(`SELECT front_order_id FROM front_orders WHERE front_order_id=?`, frontOrderID).Scan(&existingFO)
			if existingFO != "" {
				result.Skipped++
				continue
			}
			...
			// 1. 创建 front_order
			if _, err := db.Exec(`
				INSERT INTO front_orders(front_order_id, front_type, front_subtype, owner_pubkey_hex, 
			...
			// 3. 创建 trigger
			if _, err := db.Exec(`
				INSERT INTO business_triggers(trigger_id, business_id, trigger_type, trigger_id_value, trigger_role, created_at_unix, note, payload_json)
			...
			if _, err := db.Exec(`
				INSERT INTO business_settlements(settlement_id, business_id, settlement_method, status, target_type, target_id, created_at_unix, updated_at_unix, payload_json)
		...
		if err := rows.Err(); err != nil {
			return fmt.Errorf("iterate chain_payments: %w", err)
		}
		return nil
	})
...
func BackfillPoolAllocationHistory(ctx context.Context, store *clientDB) (*BackfillResult, error) {
	...
	err := store.Do(ctx, func(db *sql.DB) error {
		...
		rows, err := db.Query(`
			...
			JOIN pool_sessions ps ON ps.pool_session_id = pa.pool_session_id
			WHERE pa.allocation_kind = 'pay'
				AND pa.id = (
					-- 取该 session 的第一次 pay allocation
					SELECT MIN(id) FROM pool_allocations 
		...
		`)
		if err != nil {
			return fmt.Errorf("query pool_allocations: %w", err)
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			err := db.QueryRow(`
				SELECT settlement_id FROM business_settlements 
		...
		if err := rows.Err(); err != nil {
			return fmt.Errorf("iterate pool_allocations: %w", err)
		}
		return nil
	})
