...
func TestPhase2_OpenIsProcessFact(t *testing.T) {
	...
	// 验证：settle_process_events 中存在过程事件
	var processCount int
	if err := db.QueryRow(`SELECT COUNT(1) FROM settle_process_events WHERE process_id=?`, "proc_c2c_transfer_"+sessionID).Scan(&processCount); err != nil {
...
func TestPhase2_CloseIsProcessFact(t *testing.T) {
	...
	// 验证：settle_process_events 中存在过程事件
	var processCount int
	if err := db.QueryRow(`SELECT COUNT(1) FROM settle_process_events WHERE process_id=? AND accounting_subtype='close'`, "proc_c2c_transfer_"+sessionID).Scan(&processCount); err != nil {
		t.Fatalf("query settle_process_events failed: %v", err)
	}
	if processCount == 0 {
...
func TestPhase2_PayProcessEventUsesProcessID(t *testing.T) {
	...
	// 验证：settle_process_events 中存在过程事件
	var processCount int
	if err := db.QueryRow(`SELECT COUNT(1) FROM settle_process_events WHERE process_id=? AND accounting_subtype='chunk_pay'`, "proc_c2c_transfer_"+sessionID).Scan(&processCount); err != nil {
		t.Fatalf("query settle_process_events failed: %v", err)
	}
	if processCount == 0 {
