...
func (s *httpAPIServer) handleBizOrderPayBSV(w http.ResponseWriter, r *http.Request) {
	...
	var req BizOrderPayBSVRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid json"})
		return
	}
	resp, err := TriggerBizOrderPayBSV(r.Context(), s.store, s.rt, req)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": err.Error(), "result": resp})
		return
	}
	writeJSON(w, http.StatusOK, resp)
}
